        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <form role="form" method="POST" class="form-horizontal"
                      action="{{url("/modules/escolar/carrera/modify")}}">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                    <input type="hidden" name="idC" id="idC" value="{{$carrera->getId()}}">
                        ...
                        <div class="col-sm-5">
                            <select name="plantel" id="plantel" class="form-control">
                                @foreach($planteles as $plantel)
                                    @if($carrera->getCctPlantel() == $plantel->cct)
                                        <option value="{{$plantel->cct}}" selected>{{$plantel->nombre}}</option>
                                    @else
                        ...
                        <div class="col-sm-5">
                            <select name="modalidad" id="modalidad" class="form-control">
                                @foreach($modalidad as $mod)
                                    @if($carrera->getIdModalidad() == $mod->id)
                                        <option value="{{$mod->id}}" selected>{{$mod->descripcion}}</option>
                                    @else
